...
function ReplyPopup({ headpost, replyTarget, resolveContact, onConfirm, onClose }) {
  const [message, setMessage] = useState('')
  const [error, setError] = useState('')

  const targetAuthor = resolveContact(replyTarget.author)
...
function ReplyNode({ post, fullPostMap, resolveContact, setReplyTarget }) {
  const [expanded, setExpanded] = useState(false)
  const author = resolveContact(post.author)
  const hasChildren = post.childPostUuids.length > 0

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25em' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15em', fontSize: '0.9em' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.4em' }}>
          ...
          {author.imageUrl && (
            <img
              ...
              title={author.email}
              referrerPolicy="no-referrer"
              style={{ width: '18px', height: '18px', borderRadius: '50%', objectFit: 'cover', flexShrink: 0, cursor: 'default', alignSelf: 'center' }}
            />
          )}
          ...
          <button
            ...
            style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.78em', color: '#1a73e8', padding: '0.2em 0', flexShrink: 0 }}
          >
            Reply
          </button>
        </div>
      ...
      {expanded && hasChildren && (
        <div style={{ borderLeft: '2px solid #ddd', paddingLeft: '0.75em', display: 'flex', flexDirection: 'column', gap: '0.5em', marginLeft: '0.6em' }}>
          {post.childPostUuids.map((uuid) => {
            ...
            return (
              <ReplyNode key={uuid} post={child} fullPostMap={fullPostMap} resolveContact={resolveContact} setReplyTarget={setReplyTarget} />
            )
          })}
        </div>
      )}
